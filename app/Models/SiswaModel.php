...
class SiswaModel extends Model
{
   ...
   public function createSiswa($nis, $nama, $idKelas, $jenisKelamin, $noHp, $foto)
   {
      return $this->save([
         'nis' => $nis,
         'nama_siswa' => $nama,
   ...
   public function updateSiswa($id, $nis, $nama, $idKelas, $jenisKelamin, $noHp, $foto)
   {
      return $this->save([
         $this->primaryKey => $id,
         'nis' => $nis,
   ...
   public function updateFotoSiswa($id, $foto)
   {
   ...
   {
      log_message('error', 'Searching for NIS: ' . $nis);
   ...
   // ZIP
   public function importZIP($zipFileName)
   {
      $zipFilePath = FCPATH . 'uploads/tmp/' . $zipFileName;
      $extractPath = FCPATH . 'uploads/tmp/extracted/';
      ...
      if ($zip->open($zipFilePath) === TRUE) {
         ...
         $zip->close();
         // Proses setiap file yang diekstrak
         $files = scandir($extractPath);
         foreach ($files as $file) {
            if (pathinfo($file, PATHINFO_EXTENSION) == 'csv') {
               $this->importCSVItem($file, 1); // Mengimpor item pertama dari setiap CSV
            }
         }

         // Bersihkan file yang diekstrak
